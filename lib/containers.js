...
const { Logger, withTimeout } = require('../utils');
...
function hasPathTo(bot, pos, { approachRange = 2, pathCheckTimeout = 500 } = {}) {
  const goal = new goals.GoalNear(pos.x, pos.y, pos.z, approachRange);
  const result = bot.pathfinder.getPathTo(bot.pathfinder.movements, goal, pathCheckTimeout);
